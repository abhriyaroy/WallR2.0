...
public class MaterialSpinner extends TextView {
  ...
  public <T> void setItems(@NonNull List<T> items) {
    adapter =
        new MaterialSpinnerAdapter<>(getContext(), items).setBackgroundSelector(backgroundSelector)
            .setTextColor(textColor);
  ...
  public void setAdapter(@NonNull ListAdapter adapter) {
    this.adapter = new MaterialSpinnerAdapterWrapper(getContext(), adapter).setBackgroundSelector(
        backgroundSelector)
        .setTextColor(textColor);
  ...
  /**
   * Converts the height in dp to pixels
   **/
  public static double convertDpToPixel(Context context) {
    double dp = 54.0;
    Resources resources = context.getResources();
    DisplayMetrics metrics = resources.getDisplayMetrics();
    double px = dp * (metrics.densityDpi / DisplayMetrics.DENSITY_DEFAULT);
    return px;
  }
  ...
  private void animateArrow(boolean shouldRotateUp) {
    //swapped values here as my spinner goes up
  ...
  private int calculatePopupWindowHeight() {
    ...
    float listViewHeight =
        adapter.getCount() * getResources().getDimension(R.dimen.ms__item_height);
